test_that("compareCompositionPlot works", {
    ...
    df <- data.frame(celltype = sample(LETTERS[1:3], 100, replace = TRUE,
                                       prob = c(0.1, 0.3, 0.6)))
    idx <- 1:20
    expect_error(compareCompositionPlot(df = "error", idx = idx, column = "celltype"), 
                 "'column' is not in")
    expect_error(compareCompositionPlot(df = df, idx = "error", column = "celltype"), 
                 "of class 'numeric'")
    expect_error(compareCompositionPlot(df = df, idx = idx, column = 1), 
                 "of class 'character'")
    expect_error(compareCompositionPlot(df = df, idx = list(idx, idx), column = "celltype"), 
                 "must not be NULL")
